...
- **Routing:** file-based via `expo-router` under `src/app`. Path alias `@/*` → `src/*`, `@/assets/*` → `assets/*`.
  Root `_layout.tsx` is a Stack (tabs group + modal routes); `(tabs)/_layout.tsx` is the 4-tab bar.
- **State:** single zustand store in `src/lib/store.ts`, persisted to AsyncStorage. Local-first, single user.
  ...
  (`getAllIngredients` / `getIngredient`). Persisted keys: `userIngredients`, `recipes`, `log`, `goals`.
...
- Inline references inside all of the above — file paths (`src/app/(tabs)/…`, `src/lib/*`,
  `scripts/build-seed.mjs`), symbol names (`getAllIngredients` / `getIngredient` /
  `resolveEntryNutrients` / `recipePerServing`), persisted store keys (`userIngredients`,
